...
public class ExcelHelperImpl implements ExcelHelper {
    ...
    public HSSFWorkbook createExcelWorkBook(List<Root> rootObjects) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            createExcelWorkBook(rootObjects.get(i), delimiter, sheet);

        }
        return workbook;
    }

    private void createExcelWorkBook(Root issues, int delimiter, HSSFSheet sheet) {
        for (int i = 0; i < issues.getIssues().size(); i++) {
            Issue issue = issues.getIssues().get(i);
            ...
            HSSFRow row = sheet.createRow((short) i + 1 + delimiter);
            row.createCell(0).setCellValue(issueID);
            row.createCell(1).setCellValue(issueType);
            row.createCell(2).setCellValue(status);
            row.createCell(3).setCellValue(priority);
